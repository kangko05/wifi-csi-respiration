...
static void amp_hann(float *window, size_t n) {
    ...
    for (size_t g = 0; g < n; g++) {
        window[g] = (float)(0.5 - 0.5 * cos(2.0 * pi * (double)g / (double)n));
    }
}
...
int csi_amp_process(const csi_frame_t *frames, size_t n_frames,
    ...
    if (!amp_analysis_valid(&context->analysis, context->filter_info.output_fs))
        return 1;
    ...
    hann = malloc(n_reduced * sizeof(*hann));
    ...
    for (size_t k = 0; k < n_bins; k++) {
        result->bins[k].eligible = eligible[k];
        if (!eligible[k]) {
            result->bins[k].reasons = CSI_AMP_EXCLUDED;
            continue;
        }
        ...
        for (size_t g = 0; g < n_reduced; g++) {
            ...
            filtered[g * n_bins + k] =
                ((float)(filtered[g * n_bins + k] - trend));
        }
        ...
        if (sqrt(energy / n_reduced) <=
            context->analysis.flat_relative_floor * fmax(1.0, median))
            result->bins[k].reasons = CSI_AMP_FLAT;
    }

    const size_t fft_oversampling = context->analysis.fft_oversampling;
    ...
    fft_re = malloc(fft_capacity * sizeof(*fft_re));
    fft_im = malloc(fft_capacity * sizeof(*fft_im));

    if (n_fft / 2 + 1 > SIZE_MAX / sizeof(*psd))
        goto cleanup;

    psd = malloc((n_fft / 2 + 1) * sizeof(*psd));

    if (fft_re == NULL || fft_im == NULL || psd == NULL)
        goto cleanup;
    ...
    for (size_t k = 0; k < n_bins; k++) {
        ...
        if (!eligible[k] || (bin->reasons & CSI_AMP_FLAT))
            continue;
