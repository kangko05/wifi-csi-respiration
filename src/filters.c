...
int butter_bandpass_zero_phase(double *x, size_t n, double fs,
    ...
    if (!x || n <= 27 || n > SIZE_MAX / sizeof(double) - 54 ||
        !isfinite(fs) || !isfinite(low_hz) || !isfinite(high_hz) || fs <= 0)
        return 1;
    high_hz = fmin(high_hz, .99 * fs / 2);
    if (!(0 < low_hz && low_hz < high_hz)) return 1;
    double lo = 4 * tan(PI * low_hz / fs);
    double hi = 4 * tan(PI * high_hz / fs);
    double bw = hi - lo, center2 = lo * hi;
    double complex poles[4], denominator = 1;
    int count = 0;
    for (int k = 0; k < 4; ++k) {
        double complex p = -cexp(I * PI * (2*k-3) / 8) * bw / 2;
        ...
        for (int sign = -1; sign <= 1; sign += 2) {
            ...
            if (cimag(digital) > 0) poles[count++] = digital;
        }
    }
    if (count != 4) return 1;
    ...
    for (int i=0; i<4; ++i) for (int j=i+1; j<4; ++j)
        if (cabs(poles[j]) < cabs(poles[i])) {
            double complex tmp=poles[i]; poles[i]=poles[j]; poles[j]=tmp;
        }
    double sos[4][5], zi[4][2], scale=1;
    for (int j=0; j<4; ++j) {
        /* Four analog zeros at zero contribute (2*fs_normalized)^4. */
        double gain = j == 0 ? pow(4*bw,4)/creal(denominator) : 1;
        sos[j][0]=gain; sos[j][1]=(j<2 ? 2 : -2)*gain; sos[j][2]=gain;
        sos[j][3]=-2*creal(poles[j]);
        sos[j][4]=creal(poles[j]*conj(poles[j]));
        double dc=(sos[j][0]+sos[j][1]+sos[j][2])/(1+sos[j][3]+sos[j][4]);
    ...
    for (size_t i=0; i<27; ++i) extended[i]=2*x[0]-x[27-i];
    for (size_t i=0; i<n; ++i) extended[i+27]=x[i];
    for (size_t i=0; i<27; ++i) extended[n+27+i]=2*x[n-1]-x[n-2-i];
...
size_t firwin_coeff(const firwin_config_t *config, float *coeffs,
    ...
    for (size_t n = 0; n < n_taps; n++) {
        ...
        double h = (n == half) ? cutoff : sin(PI * cutoff * m) / (PI * m);
