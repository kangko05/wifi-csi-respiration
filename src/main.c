...
static int synthetic_demo(void) {
    ...
    for (size_t i = 0; i < n_frames; i++) {
        ...
        frames[i].re[0] = (float)(10.0 + sin(2.0 * acos(-1.0) * 0.25 * t));
        frames[i].im[0] = 0;
        frames[i].re[1] = 10;
        frames[i].im[1] = 0;
        frames[i].valid[2] = 0;
...
static int run_stdin(int phase_method) {
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        if (rssi < -128 || rssi > 127 || noise < -128 || noise > 127 ||
            ...
            format > 255 || invalid > 255 || sig_len > 65535 ||
            len > CSI_BUF_MAX)
            goto cleanup;
        ...
        if (phase_method && (len != 234 || format != 2)) goto cleanup;
    ...
    if (phase_method) {
        csi_phase_result_t phase;
        if (csi_phase_process(frames,used,&phase)) {
            printf("{\"method\":\"phase\",\"status\":\"process_error\",\"n_frames\":%zu}\n",used);
            goto cleanup;
        }
        printf("{\"method\":\"phase\",\"status\":\"%s\",\"n_frames\":%zu,"
               "\"preprocess_rejected\":%zu,\"n_samples\":%zu,\"n_retained\":%zu,"
               "\"fs\":%.17g,\"start_s\":%.17g,\"duration_s\":%.17g,\"max_gap_s\":%.17g,"
               "\"accepted\":%d,\"reasons\":%u,\"diagnostic_bpm\":%.17g,"
               "\"spectral_bpm\":%.17g,\"peak_count_bpm\":%.17g,\"sharpness\":%.17g,"
               "\"n_peaks\":%zu,\"peak_fallback\":%d,\"agreement_bpm\":",
               phase.accepted ? "periodic_candidate" : "weak_evidence",used,rejected,
               phase.n_samples,phase.n_retained,phase.sample_rate_hz,phase.start_s,
               phase.duration_s,phase.max_gap_s,phase.accepted,phase.reasons,phase.bpm,
               phase.spectral_bpm,phase.peak_count_bpm,phase.sharpness,phase.n_peaks,phase.peak_fallback);
        if (isfinite(phase.agreement_bpm)) printf("%.17g",phase.agreement_bpm);
        else printf("null");
        printf(",\"selected_bins\":[%d,%d,%d,%d,%d]}\n",phase.selected_bins[0],
               phase.selected_bins[1],phase.selected_bins[2],phase.selected_bins[3],phase.selected_bins[4]);
        status=0;
        goto cleanup;
    }
...
int main(int argc, char **argv) {
    if (argc == 2 && strcmp(argv[1], "--stdin") == 0)
        return run_stdin(0);
    if (argc == 3 && strcmp(argv[1], "--stdin") == 0 && strcmp(argv[2], "--phase") == 0)
        return run_stdin(1);
