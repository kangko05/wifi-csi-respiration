...
- 2026-09-22: 온보드 작업을 보류하고 Python 파일을 루트로 복원했다. 현재 브랜치는 `python-resume`이며 이전 C 구현·실측·전체 문서는 원격 `archive/onboard-20260922`에 보존했다. 자세한 복귀·복구 방법은 `docs/python-return-20260922.md`에 있다.
...
- `data/`: 수집 원본. C와 같이 쓰므로 **저장소 루트**에 둔다. 세션별 새 ID를 쓰며 덮어쓰지 않는다. 스크립트는 `DATA_ROOT = ROOT / "data"`로, 수집기는 `--data-root` 기본값으로 이 위치를 가리킨다.
...
- 현재 폴더가 Git 저장소 루트다(작업 브랜치 `python-resume`). 코드(`src/`, `tests/`, `scripts/`, `vendor/`, `collect_legacy.py`, `requirements.txt`)는 git으로 추적하므로 클론만으로 실행할 수 있다. 복귀 시 전체 문서와 `references/`도 명시적으로 Git에 저장한다. `data/`·일반 `outputs/`·`.venv/`는 별도 로컬 백업을 유지한다. 백업이 아니므로 별도로 보존한다.

## 실행 (저장소 루트 기준)

```bash
python3 -m venv .venv
.venv/bin/python -m pip install -r requirements.txt   # pyserial, numpy, scipy, matplotlib, threadpoolctl
