...
최종 갱신: 2026-09-22. Python 프로젝트를 저장소 루트로 복원했다. 명령은 루트에서 실행하며 원본은 `data/`, 환경은 `.venv/`에 둔다. 진입 파일은 `collect_legacy.py`이고 argparse의 `usage: collect.py`는 기존 표시 이름이다.
...
| `--data-root` | 기본 `<저장소 루트>/data` (현재 작업 폴더의 `data/`) |
