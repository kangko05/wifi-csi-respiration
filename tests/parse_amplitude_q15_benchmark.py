"""Parse completed B0 firmware measurements (four fixed-input variants)."""
import json
...
def parse(path):
    ...
    assert complete and len(rows)==4 and len(kernels)==8
    assert all(r['status']==0 for r in rows+kernels)
    assert len({r['fingerprint'] for r in rows})==1
    assert rows[-1]['name']=='b0_float_repeat' and rows[-1]['equal_baseline']==1
